...
The chart always pins `--metrics-format`, in both directions: with `prometheus.enabled=true` it sets `JSON`, the format the `mg-exporter` reads. Memgraph 3.13 changes the server-side default from `JSON` to `OpenMetrics`, so a chart that leaves the flag unset hands the exporter a payload it cannot parse. If you set both switches, `scrapeMemgraphDirectly` wins and the instances serve OpenMetrics.
